...
import { LogIn, Mail, Lock, AlertCircle, ArrowRight, UserCheck, Shield } from 'lucide-react';
...
export const LoginPage: React.FC = () => {
  ...
  const handleEmailLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (err.code === 'auth/invalid-credential' || err.code === 'auth/user-not-found' || err.code === 'auth/wrong-password') {
        ...
        // If provider not enabled in console, fall back to isolated guest auth
        try {
          await signInAnonymously(auth);
          navigate('/app');
        } catch (anonErr) {
          const emailClean = email.trim().toLowerCase() || 'usuario@ride.ia';
          const emailHash = btoa(emailClean).replace(/[^a-zA-Z0-9]/g, '').substring(0, 16);
          const customUid = 'usr_' + (emailHash || Date.now().toString());
          const customSession = {
            uid: customUid,
            name: emailClean.split('@')[0] || 'Usuário RIDE.IA',
            email: emailClean,
            role: 'user',
  ...
  const handleGuestLogin = async () => {
    ...
    try {
      ...
      console.error('Erro Guest Auth:', err);
      const customUid = 'guest_' + Date.now() + '_' + Math.random().toString(36).substring(2, 8);
      const customSession = {
        uid: customUid,
        name: 'Convidado RIDE.IA',
        email: 'convidado@ride.ia',
        role: 'user',
        createdAt: new Date().toISOString(),
      };
      localStorage.setItem('ride_custom_session', JSON.stringify(customSession));
      ...
      <div className="w-full max-w-md">
        ...
        <div className="card-surface p-6 sm:p-8 rounded-2xl border border-[#26262B] bg-[#0A0A0B] space-y-5">
          ...
          <div className="relative text-center">
            ...
            <span className="relative bg-[#0A0A0B] px-3 text-[11px] text-[#5E5E68]">ou entre com</span>
          </div>
